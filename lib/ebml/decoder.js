...
EbmlDecoder.prototype.readTag = function() {
    ...
    var info = this.getSchemaInfo(tagStr);
    tagObj = {
        tag: tag.value,
        tagStr: tagStr,
        type: info.type,
...
EbmlDecoder.prototype.closeTags = function(closeLevel) {
    if (typeof (closeLevel) === 'undefined')
        closeLevel = Number.MAX_VALUE;
    while (this._tag_stack.length > 0) {
        var topEle = this._tag_stack[this._tag_stack.length - 1];
        if ((topEle.end < 0 && topEle.level < closeLevel) ||
            this._total < topEle.end) {
            return;
        }
